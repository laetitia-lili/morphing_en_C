...
CouplesPoints* interface_selection_points(Image *img1, Image *img2) {
    ...
    // mode WINDOW : tout est dessiné en mémoire puis affiché d'un coup avec affiche_all()
    // (le mode CANVAS par défaut réaffiche la fenêtre à chaque pixel dessiné, ce qui est très lent)
    set_mode_WINDOW();
