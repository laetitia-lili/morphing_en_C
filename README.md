...
La triangulation, c'est le principe qui permet à l'image de se déformer de façon cohérente plutôt que de simplement fondre en transparence d'une image à l'autre.

Concrètement :
1. Les points cliqués sur chaque image (yeux, nez, contours…) sont reliés entre eux pour former un maillage de **triangles** qui recouvre toute l'image.
2. Comme les mêmes couples de points existent sur l'image de départ et sur l'image d'arrivée, chaque triangle de l'image 1 a un triangle correspondant dans l'image 2 — mais avec une forme et une position différentes.
3. Pour chaque image intermédiaire, on calcule la position et la forme "à mi-chemin" de chaque triangle (interpolation linéaire des sommets), puis on remplit chaque triangle intermédiaire avec les couleurs interpolées entre le triangle source et le triangle destination (interpolation barycentrique).
4. En répétant l'opération sur tous les triangles et pour chaque étape du morphing, l'image entière se déforme et se colore progressivement pour passer d'une image à l'autre.

C'est cette étape par étape sur des triangles (plutôt qu'un simple fondu pixel à pixel) qui donne l'effet de morphing réaliste, où les traits du visage (ou de l'objet) semblent vraiment se transformer plutôt que juste apparaître/disparaître en transparence.
